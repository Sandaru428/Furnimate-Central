...
import { paymentsAtom, currencyAtom, saleOrdersAtom } from '@/lib/store';
  ...
  const [isPaymentDialogOpen, setIsPaymentDialogOpen] = useState(false);
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            const [paymentsSnapshot, soSnapshot] = await Promise.all([
                ...
                <TableRow>
                  ...
                  <TableHead>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" className="h-8 p-0 hover:bg-transparent flex flex-col items-start">
                          <span>Customer <ChevronDown className="ml-1 h-3 w-3 inline" /></span>
                          {customerFilter !== 'all' && <span className="text-xs text-muted-foreground font-normal">({customerFilter})</span>}
                        </Button>
                      </DropdownMenuTrigger>
                        ...
                        return (
                            <TableRow key={payment.id}>
                                ...
                                <TableCell className="text-right text-green-600">
                                    +{currency.code} {payment.amount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </TableCell>
